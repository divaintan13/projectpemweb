  ...
  <div class="form-group row">
    ...
    <div class="col-8">
      <select id="pasien_id" name="pasien_id" class="custom-select" required="required">
        ...
        <?php foreach ($pasien_id as $pas) : ?>
            <option <?= $pasien["pasien_id"] == $pas['id'] ? 'selected' : '' ?> value="<?= $pas['id'] ?>"><?= $pas['nama'] ?></option>
          <?php endforeach; ?>
      </select>
    </div>
  </div>
  <div class="form-group row">
    ...
    <div class="col-8">
      <select id="dokter_id" name="dokter_id" class="custom-select" required="required">
        ...
        <?php foreach ($dokter_id as $dok) : ?>
            <option <?= $pasien["dokter_id"] == $dok['id'] ? 'selected' : '' ?> value="<?= $dok['id'] ?>"><?= $dok['nama'] ?></option>
          <?php endforeach; ?>
      </select>
    </div>
  </div>
